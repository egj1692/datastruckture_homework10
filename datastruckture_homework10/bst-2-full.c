...
{
	/* 재귀함수를 활용하여 왼쪽 child가 없을 때까지 들어가서
	 * 왼쪽 child를 출력한다. 그리고 출력 후 오른쪽 child가 있으면 들어가서
	 * 계속 재귀함수를 활용하여 왼쪽 child를 먼저 출력한다.
	 */
	if(ptr) {
...
void iterativeInorder(Node* node)
{
	...
	for(;;)
	{
		/* 왼쪽 child가 없을 때까지 stack에 넣기 */
		for(; node; node = node->left)
			push(node);

		/* 가장 왼쪽 child까지 stack에 넣으면
		 * pop하여 가장 왼쪽 child 출력
		 */
		node = pop();

		/* pop했을 때, node가 없으면 무한 루프에서 벗어나 함수 종료 */
		if(!node) break;
		/* node가 있으면 key값 출력 */
		printf(" [%d] ", node->key);
